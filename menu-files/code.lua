	...
	function ESPOptionMenu()
		...
		if(!espopsopen) then
			hook.Add("Think", "espoptionhook", function()
				...
				--ShowArmor
				local ShowArmorBox = vgui.Create("DCheckBoxLabel", ESPOptionFrame)-- Create the checkbox
				ShowArmorBox:SetPos( 50, 240 )						-- Set the position
				ShowArmorBox:SetText("Show Armor")					-- Set the text next to the box
				if(showarmor) then
					ShowArmorBox:SetValue(true)
				else
					ShowArmorBox:SetValue(false)
				end
				ShowArmorBox:SizeToContents()		
				ShowArmorBox.OnChange = function()
					showarmor = !showarmor
				end
				function ESPOptionFrame:OnClose()
					espopsopen=false
	...
	function getHealth( ply, cmd, args )
		...
		for k, v in ipairs( player.GetAll() ) do
			if string.find( string.lower( v:Nick() ), nick ) then
				...
				--Chat Output
				chat.AddText("[IWAN] Player Database Information about " .. v:Nick() )
				chat.AddText("[IWAN] IsAlive?: " .. Alive )
				chat.AddText("[IWAN] Health: " .. v:Health() )
				chat.AddText("[IWAN] Armor: " .. v:Armor() )
				chat.AddText("[IWAN] Max Armor: " .. v:GetMaxArmor() )
				chat.AddText("[IWAN] Deaths: " .. v:Deaths() )
				chat.AddText("[IWAN] Active Weapon: " .. v:GetActiveWeapon():GetClass() )
				chat.AddText("[IWAN] SteamID: " .. v:SteamID() )
				chat.AddText("[IWAN] SteamID64: " .. v:SteamID64() )
				chat.AddText("[IWAN] Is Admin?: " .. Admin )
